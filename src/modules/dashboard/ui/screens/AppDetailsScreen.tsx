...
export const AppDetailsScreen = () => {
  ...
  const [appVersion, setAppVersion] = useState("1.1.10-beta");
  ...
  useEffect(() => {
    ...
    if (window.ipcRenderer) {
      ...
      const handleUpdateStatus = (
        ...
        if (
          status === "error" &&
          (lowerMsg.includes("no published versions") ||
  ...
  const handleClearStats = () => {
    triggerConfirm(
      "Limpar Estatísticas",
      "Deseja limpar todo o histórico de coletas? Esta ação não pode ser desfeita.",
      async () => {
        ...
        try {
          ...
          if (!userId) {
            ...
            if (pbAuth) {
              try {
                const d = JSON.parse(pbAuth);
                userId = d.model?.id;
                if (d.token) pb.authStore.save(d.token, d.model);
              } catch {
                /* empty */
              }
            }
  ...
  const handleInstallUpdate = () => {
    if (window.ipcRenderer) window.ipcRenderer.send("quit-and-install-update");
  };
  ...
  const statusDot = (ok: boolean) => (
    <span
      style={{
        ...
        height: 7,
        borderRadius: "50%",
        background: ok ? "#4caf50" : "#cc3333",
        boxShadow: ok
          ? "0 0 5px rgba(76,175,80,0.6)"
          : "0 0 5px rgba(204,51,51,0.6)",
        flexShrink: 0,
      }}
    />
  );
        ...
        >
          <div
            style={{
              ...
              width: 3,
              height: "100%",
              background: "linear-gradient(180deg,#c8860a,#7a4e08)",
            }}
          />
          {/* Corner brackets */}
          <div
            style={{
              ...
              width: 10,
              height: 10,
              borderTop: "1px solid #c8860a",
              borderLeft: "1px solid #c8860a",
            }}
          />
          <div
            style={{
          ...
          >
            <img
              src="./images/sunagakure_logo.svg"
              alt="Sunagakure Logo"
          ...
          >
            {updateStatus === "idle" && (
              ...
              >
                <div>
                  <div
                    style={{ fontSize: 12, fontWeight: 700, color: "#f0e8c0" }}
                  >
                    Verificar atualizações
                  </div>
                  <div style={{ fontSize: 10, color: "#7a8060", marginTop: 3 }}>
                    Verifique se há novas versões
                  </div>
                </div>
                <button
            ...
            {updateStatus === "checking" && (
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div>
                  <div
                    style={{ fontSize: 11, fontWeight: 600, color: "#f0e8c0" }}
                  >
            ...
            {updateStatus === "not-available" && (
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <div>
                  ...
                  >
                    v{appVersion} é a última versão
                  </div>
                </div>
              </div>
            )}
            {updateStatus === "error" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                ...
                >
                  <span
                    style={{ fontSize: 11, fontWeight: 700, color: "#cc3333" }}
                  >
                    Falha ao atualizar
                  </span>
                  <button
                ...
                >
                  {updaterError || "Erro ao conectar ao servidor."}
                </p>
              </div>
            )}
          </div>
