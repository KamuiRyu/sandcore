    ...
    <AppModal
      ...
      footer={
        <div className="flex justify-end w-full">
          <button
            className={cn(
              'inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/5 px-6 font-mono text-[0.68rem] font-black uppercase tracking-[0.08em] text-slate-100 transition duration-200 hover:-translate-y-px hover:border-cyan-500/40 hover:bg-cyan-950/20 cursor-pointer active:scale-95'
            )}
            onClick={onClose}
            type="button"
          >
      ...
      <div className="flex flex-col sm:flex-row h-[500px]">
        ...
        <div className="w-full sm:w-[200px] border-b sm:border-b-0 sm:border-r border-white/10 bg-white/[0.02] p-3 flex sm:flex-col gap-1.5 shrink-0">
          <button
            ...
            className={cn(
              "flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 text-left cursor-pointer",
              activeTab === 'notifications' 
                ? "bg-[var(--cyan)] text-slate-950 font-black shadow-[0_0_20px_rgba(0,214,163,0.2)]" 
          ...
          {activeTab === 'notifications' ? (
              ...
              <div className="grid gap-5">
                <div className="flex items-center justify-between gap-4">
                  <div className="grid gap-1">
                    ...
                    <p className="text-[11px] text-slate-500">Tocar efeitos sonoros quando os timers expirarem.</p>
                  </div>
                  <button
                    type="button"
                    ...
                    className={cn(
                      ...
                      settings.soundEnabled ? 'bg-[var(--cyan)]' : 'bg-slate-800'
                    )}
                  >
                ...
                {settings.soundEnabled && (
                  <div className="grid gap-5 p-4 rounded-2xl bg-white/[0.03] border border-white/5">
                    <div className="grid gap-2">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <button onClick={testSound} className="grid h-10 w-10 place-items-center rounded-xl bg-white/5 border border-white/10 text-white hover:text-[var(--cyan)] transition-all cursor-pointer"><Play size={14} fill="currentColor" /></button>
                      </div>
                    </div>
                    <div className="grid gap-2">
                      <label className="text-[10px] font-black uppercase text-slate-500 flex items-center gap-2"><Volume2 size={12} /> Volume ({Math.round(settings.soundVolume * 100)}%)</label>
                      <input type="range" min="0" max="1" step="0.05" value={settings.soundVolume} onChange={(e) => onUpdate({ soundVolume: parseFloat(e.target.value) })} className="w-full h-1 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-[var(--cyan)]" />
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="grid gap-6">
                <div className="flex items-center justify-between gap-4">
                  <div className="grid gap-1">
                    ...
                    <p className="text-[11px] text-slate-500">Alertas mesmo se a aba estiver em segundo plano.</p>
                  </div>
                  {isBrowserPushSupported ? (
                    <button type="button" onClick={() => settings.pushEnabled ? onUpdate({ pushEnabled: false }) : onRequestPushPermission()} className={cn('relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors', settings.pushEnabled ? 'bg-[var(--cyan)]' : 'bg-slate-800')}><span className={cn('inline-block h-5 w-5 transform rounded-full bg-white transition', settings.pushEnabled ? 'translate-x-5' : 'translate-x-0')} /></button>
              ...
              <div className="grid gap-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2 font-mono text-[9px] font-black uppercase">
                    <button onClick={() => toggleAllTypes(true)} className="text-[var(--cyan)] hover:brightness-125 cursor-pointer bg-transparent border-0 outline-none">Todos</button>
                    <span className="opacity-20 text-white">|</span>
                    <button onClick={() => toggleAllTypes(false)} className="text-slate-500 hover:text-slate-300 cursor-pointer bg-transparent border-0 outline-none">Nenhum</button>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-y-2.5 max-h-40 overflow-y-auto pr-2 custom-scrollbar">
                  <div className="flex items-center gap-3"><Checkbox id="cat-custom" checked={settings.enabledTypes['custom'] !== false} onCheckedChange={() => toggleType('custom')} /><label htmlFor="cat-custom" className="text-xs font-medium text-slate-300 cursor-pointer select-none">Pinos Customizados</label></div>
                  {markerTypes.filter(t => !uncompletableTypes.includes(t)).map(type => (
                    <div key={type} className="flex items-center gap-3"><Checkbox id={`cat-${type}`} checked={settings.enabledTypes[type] !== false} onCheckedChange={() => toggleType(type)} /><label htmlFor={`cat-${type}`} className="text-xs font-medium text-slate-400 cursor-pointer select-none">{getMarkerTypeLabel(type)}</label></div>
                  ))}
                </div>
              </div>
            </div>
            ...
            <div className="grid gap-8 animate-[fade-in_150ms_ease-out]">
              <div className="grid gap-6">
                ...
                <div className="grid gap-4">
                  <div className="flex items-center justify-between p-4 rounded-2xl bg-white/[0.03] border border-white/5 group hover:border-white/10 transition-all">
                    <div className="grid gap-1">
                      ...
                      <p className="text-[10px] text-slate-500 leading-relaxed">Sugere automaticamente o último recurso coletado naquele tipo de node.</p>
                    </div>
                    <button onClick={() => onUpdate({ rememberLastSubtype: !settings.rememberLastSubtype })} className={cn('relative inline-flex h-5 w-10 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-all', settings.rememberLastSubtype ? 'bg-[var(--cyan)]' : 'bg-slate-800')}><span className={cn('inline-block h-4 w-4 transform rounded-full bg-white transition', settings.rememberLastSubtype ? 'translate-x-5' : 'translate-x-0')} /></button>
                  </div>
                  ...
                  <div className="flex items-center justify-between p-4 rounded-2xl bg-white/[0.03] border border-white/5 group hover:border-white/10 transition-all">
                    <div className="grid gap-1">
                      ...
                      <p className="text-[10px] text-slate-500 leading-relaxed">Ocultar spots que não possuem marcação ou timer ativo (limpeza visual).</p>
                    </div>
                    <button onClick={() => onUpdate({ hideUnmarkedResources: !settings.hideUnmarkedResources })} className={cn('relative inline-flex h-5 w-10 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-all', settings.hideUnmarkedResources ? 'bg-[var(--cyan)]' : 'bg-slate-800')}><span className={cn('inline-block h-4 w-4 transform rounded-full bg-white transition', settings.hideUnmarkedResources ? 'translate-x-5' : 'translate-x-0')} /></button>
                  </div>
                  ...
                  <div className="flex items-center justify-between p-4 rounded-2xl bg-white/[0.03] border border-white/5 group hover:border-white/10 transition-all">
                    <div className="grid gap-1">
                      ...
                      <p className="text-[10px] text-slate-500 leading-relaxed">Exibir selo ninja laranja e brilho em recursos que terminaram o tempo.</p>
                    </div>
                    <button onClick={() => onUpdate({ showReadyAlerts: !settings.showReadyAlerts })} className={cn('relative inline-flex h-5 w-10 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-all', settings.showReadyAlerts ? 'bg-[var(--cyan)]' : 'bg-slate-800')}><span className={cn('inline-block h-4 w-4 transform rounded-full bg-white transition', settings.showReadyAlerts ? 'translate-x-5' : 'translate-x-0')} /></button>
                  </div>
                  ...
                  <div className="flex items-center justify-between p-4 rounded-2xl bg-white/[0.03] border border-white/5 group hover:border-white/10 transition-all">
                    <div className="grid gap-1">
                      ...
                      <p className="text-[10px] text-slate-500 leading-relaxed">Exibir nomes das sub-regiões (como Vale do Fim) flutuando no mapa.</p>
                    </div>
                    <button onClick={() => onUpdate({ showSubRegionNames: settings.showSubRegionNames !== false ? false : true })} className={cn('relative inline-flex h-5 w-10 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-all', settings.showSubRegionNames !== false ? 'bg-[var(--cyan)]' : 'bg-slate-800')}><span className={cn('inline-block h-4 w-4 transform rounded-full bg-white transition', settings.showSubRegionNames !== false ? 'translate-x-5' : 'translate-x-0')} /></button>
                  </div>
                  ...
                  <div className="flex items-center justify-between p-4 rounded-2xl bg-white/[0.03] border border-white/5 group hover:border-white/10 transition-all">
                    <div className="grid gap-1">
                      ...
                      <p className="text-[10px] text-slate-500 leading-relaxed">Se ativado, novas marcações serão feitas para o Grupo por padrão.</p>
                    </div>
                    <button onClick={() => onUpdate({ defaultPinVisibility: settings.defaultPinVisibility === 'group' ? 'private' : 'group' })} className={cn('relative inline-flex h-5 w-10 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-all', settings.defaultPinVisibility === 'group' ? 'bg-yellow-500' : 'bg-slate-800')}><span className={cn('inline-block h-4 w-4 transform rounded-full bg-white transition', settings.defaultPinVisibility === 'group' ? 'translate-x-5' : 'translate-x-0')} /></button>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-yellow-500/10 bg-yellow-500/5 p-4 flex gap-4">
                 ...
                 <div className="grid gap-1">
                    ...
                    <p className="text-[10px] text-slate-400 leading-relaxed">Alertas globais de outros membros do clã estão ativos por padrão na sua conta se você estiver em um grupo.</p>
                 </div>
              </div>
            </div>
          )}
