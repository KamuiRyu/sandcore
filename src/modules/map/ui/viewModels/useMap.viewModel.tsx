...
import { defaultMapRegion, getMarkerTypeLabel, mapLayers, mapRegions, markerTypes, uncompletableTypes, getResourceTimer, getMarkerIconSrc } from '../../core/entities/MapConfig.entity'
import type { CustomRoute, MapPointReference, SavedCustomPin, SavedMapRoute } from '../../core/entities/MapRoute.entity'
...
function toOfficialPointReference(): MapPointReference[] {
  return mapPoints.map((point) => ({
    iconId: point.iconId, id: String(point.id), layerIds: point.layerIds, markerId: point.markerId, name: point.name,
    regionId: point.regionId, subRegionId: point.subRegionId, type: point.type, x: point.x, y: point.y,
    timer: point.timer || 0, description: point.description,
  }))
}

function cleanRouteForPersist(route: CustomRoute, allCustomPins: SavedCustomPin[]): CustomRoute {
  const referencedPinIds = new Set(route.checkpoints.map((c) => c.customPinId).filter(Boolean))
  const routeCustomPins = allCustomPins.filter((pin) => referencedPinIds.has(pin.id)).map((pin) => ({ 
      color: pin.color, description: pin.description, iconId: pin.iconId, id: pin.id, name: pin.name, 
      tags: pin.tags, x: pin.x, y: pin.y, imageUrl: pin.imageUrl, isPlaced: pin.isPlaced, checked: pin.checked 
    }))
  return { 
    ...route, name: route.name.trim() || 'Nova rota', description: route.description?.trim() || '', 
    customPins: routeCustomPins, 
    checkpoints: route.checkpoints.map((checkpoint) => ({ ...checkpoint, label: checkpoint.label?.trim() || undefined })) 
  }
}
...
export function useMapViewModel() {
  ...
  const [selectedLayers, setSelectedLayers] = useState<MapLayerId[]>(['officialPins', 'customPins', 'routes'])
  const [currentRoute, setCurrentRoute] = useState<CustomRoute>(() => ({ 
    checkpoints: [], color: '#00d6a3', createdAt: new Date().toISOString(), 
    customPins: [], description: '', id: createId('route'), name: 'Nova rota', updatedAt: new Date().toISOString() 
  }))
  const [selectedOfficialPointId, setSelectedOfficialPointId] = useState<string | null>(null)
  const [selectedCustomPinId, setSelectedCustomPinId] = useState<string | null>(null)
  const [savedRoutes, setSavedRoutes] = useState<SavedMapRoute[]>([])
  const [customPins, setCustomPins] = useState<SavedCustomPin[]>(() => mapDependencies.localCustomPinsStorage.read())
  const [selectedSavedRouteId, setSelectedSavedRouteId] = useState<string | null>(null)
  const [editingCustomPinId, _setEditingCustomPinId] = useState<string | null>(null)
  const editingCustomPinIdRef = useRef<string | null>(null)
  const _setEditingCustomPinIdRef = useCallback((id: string | null) => { editingCustomPinIdRef.current = id; _setEditingCustomPinId(id) }, [])
  ...
  const [dailyStats, setDailyStats] = useState<MapCollectionStats[]>(() => {
    try { const stored = localStorage.getItem('shinobi-map-stats-history'); return stored ? JSON.parse(stored) : [] } catch { return [] }
  })
  ...
  const aggregatedStats = useMemo(() => {
    ...
    const sevenDaysAgo = new Date(nowRef.getTime() - 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
    ...
    return { today: todayStats, weekly: sumStats(weeklyList), monthly: sumStats(monthlyList), total: sumStats(dailyStats) }
  }, [dailyStats])

  const currentStatsView = aggregatedStats[statsPeriod]

  const incrementResourceStat = useCallback((type: string, subType?: string) => {
    setDailyStats(prev => {
      const todayStr = new Date().toISOString().split('T')[0]
      const next = [...prev]
      let todayIdx = next.findIndex(s => s.date === todayStr)
      if (todayIdx === -1) { next.push(createEmptyStats(todayStr)); todayIdx = next.length - 1 }
      const today = { ...next[todayIdx] }
      if (type === 'ore') { 
        const key = subType || 'unidentified'
        today.ore_count = { ...today.ore_count, [key]: (today.ore_count[key] || 0) + 1 } 
      }
      else if (type === 'mushroom') {
        const key = subType || 'unidentified'
        today.mushroom_count = { ...today.mushroom_count, [key]: (today.mushroom_count[key] || 0) + 1 }
      }
      else if (type === 'stick') { today.stick_count += 1 }
      else if (['perpetual', 'hibiscus', 'cotton', 'borago'].includes(type)) { 
        today.plant_count = { ...today.plant_count, [type]: (today.plant_count[type] || 0) + 1 } 
      }
      next[todayIdx] = today; localStorage.setItem('shinobi-map-stats-history', JSON.stringify(next)); return next
    })
  }, [])
  ...
  useEffect(() => {
    ...
    syncTimerRef.current = window.setTimeout(async () => {
      ...
      if (todayStats) await mapDependencies.mapStatsRepository.saveDailyStats(user.id, todayStats)
    }, 10000)
  ...
  const [pinVisibility, setPinVisibility] = useState<'private' | 'group'>(() => mapDependencies.localNotificationSettingsStorage.read().defaultPinVisibility || 'private')
  const [globalTick, setGlobalTick] = useState(() => Date.now())

  const [searchHistory, setSearchHistory] = useState<string[]>(() => {
    try { const stored = localStorage.getItem('shinobi-map-search-history'); return stored ? JSON.parse(stored) : [] }
    catch { return [] }
  })
  ...
  const referencedOfficialPointIds = useMemo(() => {
    ...
    const allVisible = [...savedRoutes, ...publicRoutes].filter(r => visibleRoutes.includes(r.id))
    allVisible.forEach(r => r.route.checkpoints.forEach(cp => { if (cp.pointId) ids.add(cp.pointId) })); 
    if (mode === 'route') currentRoute.checkpoints.forEach(cp => { if (cp.pointId) ids.add(cp.pointId) });
    return ids
  }, [savedRoutes, publicRoutes, visibleRoutes, mode, currentRoute.checkpoints])

  const referencedCustomPinIds = useMemo(() => {
    const ids = new Set<string>(); 
    const allVisible = [...savedRoutes, ...publicRoutes].filter(r => visibleRoutes.includes(r.id))
    allVisible.forEach(r => r.route.checkpoints.forEach(cp => { if (cp.customPinId) ids.add(cp.customPinId) })); 
    if (mode === 'route') currentRoute.checkpoints.forEach(cp => { if (cp.customPinId) ids.add(cp.customPinId) });
    return ids
  }, [savedRoutes, publicRoutes, visibleRoutes, mode, currentRoute.checkpoints])
  ...
  const visibleOfficialPoints = useMemo(() => {
    ...
    const matchesTerms = (targetTexts: Array<string | undefined>) => {
      ...
      const standardTargets = targetTexts.filter(Boolean).map(t => t!.toLowerCase())
    ...
    return officialPoints.map(p => {
        ...
        if (state?.subType) {
            ...
            if (resData) return { ...p, name: resData.name, iconId: resData.iconId, isCompleted: state.status !== 'ready' }
        }
  ...
  const getLastGlobalResetTime = useCallback(() => {
    const now = new Date()
    const t00 = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 0, 0, 0).getTime()
    const t12 = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 12, 0, 0).getTime()
  ...
  const triggerAlert = useCallback((p: MapPointReference, m: string) => {
    showToast(p.type === 'merchant' ? 'Despawn' : 'Respawn', m, 'info')
    if (notificationSettings.soundEnabled) SoundSynthesizer.play(notificationSettings.soundType, notificationSettings.soundVolume)
    ...
    const interval = setInterval(() => {
      ...
      setCompletedPins((prev) => {
        ...
        for (const [id, state] of Object.entries(next)) {
          ...
          const isRes = ['ore', 'mushroom', 'stick', 'perpetual', 'hibiscus', 'cotton', 'borago'].includes(p.type)
          ...
          if (p.timer || state.subType) {
            const timer = (state.subType ? getResourceTimer(state.subType) : undefined) ?? p.timer ?? 0
              ...
              if (isRes) {
                if (state.status !== 'ready') {
                  ...
                  if (isAuthenticated && user) {
                    mapDependencies.respawnRepository.record(id, state.completedAt, p.name, user.id, timer, getMarkerIconSrc(state.subType || p.iconId), p.regionId, p.subRegionId, p.type, state.subType, 'ready', pinVisibility === 'group' ? group?.id : undefined).catch(logger.error)
                  }
                }
              } else {
                delete next[id]
            ...
            } else if (p.type !== 'merchant' && notificationSettings.enabledTypes?.[p.type] !== false) {
              if (rem <= notificationSettings.leadTime && !state.leadNotified) { triggerAlert(p, `Faltam ${rem}s para ${p.name}!`); next[id] = { ...state, leadNotified: true }; hasChanges = true }
            }
          }
        }
        if (hasChanges) {
          localStorage.setItem('shinobi-map-completed-pins', JSON.stringify(next))
          return next
        }
        return prev
      })
  ...
  const loadGroupData = useCallback(async () => {
    ...
    try {
      ...
      setGroup(myGroup)
      if (myGroup) {
        const mems = await mapDependencies.groupRepository.getGroupMembers(myGroup.id)
    ...
    mapDependencies.respawnRepository.listActive().then((records) => {
      setCompletedPins((prev) => {
        ...
        records.forEach((r) => {
          ...
          const isMyGroup = groupRef.current && r.groupId === groupRef.current.id
          if (isMine || isMyGroup) { next[r.pinId] = { completedAt: r.completedAt, subType: r.subType, status: r.status as 'cooldown' | 'ready' } }
        })
        return next
      })
    })
    const unsubscribe = mapDependencies.respawnRepository.subscribe((update) => {
      if ('deleted' in update) { setCompletedPins(prev => { const next = { ...prev }; delete next[update.pinId]; return next }) }
      else {
        const isMine = userRef.current && update.ownerId === userRef.current.id
        const isMyGroup = groupRef.current && update.groupId === groupRef.current.id
        if (isMine || isMyGroup) {
          setCompletedPins(prev => {
              const next = { ...prev }
              const existing = next[update.pinId]
              if (!existing || new Date(update.completedAt) >= new Date(existing.completedAt) || update.status !== existing.status) {
                  next[update.pinId] = { completedAt: update.completedAt, subType: update.subType, status: update.status as 'cooldown' | 'ready' }
              }
              return next
          })
        }
  ...
  useEffect(() => {
    if (isAuthenticated && user?.id) {
      mapDependencies.notificationSettingsRepository.update(user.id, notificationSettings).catch(logger.error)
    }
  ...
  const createGroup = useCallback(async (name: string) => {
    ...
    setIsGroupLoading(true)
    try {
      const newGroup = await mapDependencies.groupRepository.createGroup(name, userId)
      setGroup(newGroup)
      await loadGroupData()
  ...
  const joinGroup = useCallback(async (code: string) => {
    ...
    setIsGroupLoading(true)
    try {
      const joinedGroup = await mapDependencies.groupRepository.joinGroupByCode(code, userId)
  ...
  const copyInviteCode = useCallback(() => {
    if (group) { navigator.clipboard.writeText(group.inviteCode); showToast('Copiado', 'Código copiado!', 'success') }
  }, [group, showToast])

  const updateNotificationSettings = useCallback((s: Partial<NotificationSettings>) => {
    if (s.pushEnabled === false && notificationSettings.pushEnabled === true && isAuthenticated) {
  ...
  const togglePinCompleted = useCallback((id: string, subType?: string, isRestarting: boolean = false) => {
    const p = officialPoints.find(i => i.id === id)
    ...
    if (isUncompletable) {
      setCompletedPins((prev) => {
        ...
        else next[id] = { completedAt: new Date().toISOString(), status: 'cooldown' } // Usamos cooldown apenas para indicar 'marcado'
        return next
      })
      
      // Sincronizar com PocketBase se necessário (opcional para estáticos, mas bom para persistência)
      if (isAuthenticated && user) {
        if (isUnmarking) mapDependencies.respawnRepository.cancel(id).catch(logger.error)
        else mapDependencies.respawnRepository.record(id, new Date().toISOString(), p?.name || 'Pin', user.id, 0, getMarkerIconSrc(p!.iconId), p?.regionId, p?.subRegionId, p?.type, undefined, 'cooldown', pinVisibility === 'group' ? group?.id : undefined).catch(logger.error)
      }
      return
    }

    const isRes = p && ['ore', 'mushroom', 'stick', 'perpetual', 'hibiscus', 'cotton', 'borago'].includes(p.type)
    ...
    const timer = (subType ? getResourceTimer(subType) : undefined) ?? p?.timer ?? 0
    
    const iconUrl = subType ? getMarkerIconSrc(subType) : (p ? getMarkerIconSrc(p.iconId) : '')
    if (isAuthenticated && user) {
      const userId = user.id
      if (isUnmarking) {
          if (isRes) mapDependencies.respawnRepository.record(id, currentState.completedAt, p?.name || 'Pin', userId, timer, iconUrl, p?.regionId, p?.subRegionId, p?.type || 'custom', currentState.subType, 'ready', finalGroupId).catch(logger.error)
          else mapDependencies.respawnRepository.cancel(id).catch(logger.error)
      }
      else mapDependencies.respawnRepository.record(id, new Date().toISOString(), p?.name || 'Pin', userId, timer, iconUrl, p?.regionId, p?.subRegionId, p?.type || 'custom', subType, 'cooldown', finalGroupId).catch(logger.error)
    }
    if (!isUnmarking) {
        const isNewCollection = !currentState || currentState.subType !== subType || currentState.status === 'ready' || isRestarting
        if (isNewCollection) {
    ...
    setCompletedPins((prev) => {
      ...
      if (isUnmarking) {
          ...
          const isStandard = subType === 'ore_1' || subType === 'mushroom_1' || (currentState?.subType === 'ore_1' || currentState?.subType === 'mushroom_1')
          if (isRes && isStandard) delete next[id]
          else if (isRes) next[id] = { ...currentState, status: 'ready' }
          else delete next[id]
      }
      else { next[id] = { completedAt: new Date().toISOString(), subType, status: 'cooldown' } }
      return next
    })
  }, [officialPoints, isAuthenticated, user, completedPins, pinVisibility, group, incrementResourceStat])

  const resetAllActiveRespawns = useCallback(async () => {
    if (!confirm('Deseja resetar TODAS as marcações ativas (Pessoais e de Grupo)?')) return
    try {
        ...
        for (const record of active) {
            ...
            const isMyGroup = groupRef.current && record.groupId === groupRef.current.id
            if (isMine || isMyGroup) { await mapDependencies.respawnRepository.cancel(record.pinId) }
        }
        setCompletedPins({}); showToast('Sucesso', 'Todas as marcações foram resetadas.', 'success')
    } catch { showToast('Erro', 'Falha ao resetar marcações.', 'error') }
  }, [showToast])

  const interactiveMap = useInteractiveMap({
    onMapTap: (coords) => {
      if (editingCustomPinIdRef.current) { setDraftCustomPin(prev => prev ? { ...prev, x: coords.x, y: coords.y, isPlaced: true } : prev); return }
      if (mode === 'pin') {
        const id = createId('pin'); const p: SavedCustomPin = { color: '#00d6a3', description: '', iconId: 'pin', id, name: `Pin ${customPins.length + 1}`, tags: [], x: coords.x, y: coords.y, isPlaced: true, source: 'local', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }
        setDraftCustomPin(p); setSelectedCustomPinId(id); _setEditingCustomPinIdRef(id); return
      }
      if (mode === 'route') {
        const clickThreshold = 1.2 / interactiveMap.displayedCamera.scale
        const getDist = (p: { x: number, y: number }) => Math.sqrt(Math.pow(p.x - coords.x, 2) + Math.pow(p.y - coords.y, 2))
        
        let bestVisibleO: MapPointReference | null = null; let minVisibleO = Infinity
        visibleOfficialPoints.forEach(p => { const d = getDist(p); if (d < minVisibleO) { minVisibleO = d; bestVisibleO = p } })
        
        let bestVisibleC: SavedCustomPin | null = null; let minVisibleC = Infinity
        visibleCustomPins.forEach(p => { const d = getDist(p); if (d < minVisibleC) { minVisibleC = d; bestVisibleC = p } })
        
        let minFullO = Infinity
        officialPoints.forEach(p => { const d = getDist(p); if (d < minFullO) { minFullO = d } })
        
        let minFullC = Infinity
        customPins.forEach(p => { const d = getDist(p); if (d < minFullC) { minFullC = d } })

        const isClickingOnVisibleO = minVisibleO < clickThreshold
        const isClickingOnVisibleC = minVisibleC < clickThreshold
        const isClickingOnHiddenO = minFullO < clickThreshold && !isClickingOnVisibleO
        const isClickingOnHiddenC = minFullC < clickThreshold && !isClickingOnVisibleC

        if (isClickingOnHiddenO || isClickingOnHiddenC) return

        if (isClickingOnVisibleO && minVisibleO <= minVisibleC && bestVisibleO) {
            setCurrentRoute(prev => ({ ...prev, checkpoints: [...prev.checkpoints, { id: createId('checkpoint'), x: bestVisibleO!.x, y: bestVisibleO!.y, pointId: bestVisibleO!.id, label: bestVisibleO!.name }] }))
        } else if (isClickingOnVisibleC && bestVisibleC) {
            setCurrentRoute(prev => ({ ...prev, checkpoints: [...prev.checkpoints, { id: createId('checkpoint'), x: bestVisibleC!.x, y: bestVisibleC!.y, customPinId: bestVisibleC!.id, label: bestVisibleC!.name }] }))
        } else {
            setCurrentRoute(prev => ({ ...prev, checkpoints: [...prev.checkpoints, { id: createId('checkpoint'), x: coords.x, y: coords.y, label: `Ponto ${prev.checkpoints.length + 1}` }] }))
        }
        return
      }
      if (mode === 'feedback') { setFeedbackTarget({ x: coords.x, y: coords.y }); setIsFeedbackModalOpen(true); _setMode('explore'); return }
      if (mode === 'explore') {
        const clickThreshold = 1.2 / interactiveMap.displayedCamera.scale
        const getDist = (p: { x: number, y: number }) => Math.sqrt(Math.pow(p.x - coords.x, 2) + Math.pow(p.y - coords.y, 2))
        let bestO: MapPointReference | null = null; let minO = Infinity
        visibleOfficialPoints.forEach(p => { const d = getDist(p); if (d < minO) { minO = d; bestO = p } })
        let bestC: SavedCustomPin | null = null; let minC = Infinity
        visibleCustomPins.forEach(p => { const d = getDist(p); if (d < minC) { minC = d; bestC = p } })
        if (minO < clickThreshold && minO <= minC) { setSelectedOfficialPointId(bestO!.id); setSelectedCustomPinId(null); _setEditingCustomPinIdRef(null); setDraftCustomPin(null) }
        else if (minC < clickThreshold) { setSelectedCustomPinId(bestC!.id); setSelectedOfficialPointId(null); _setEditingCustomPinIdRef(null); setDraftCustomPin(null) }
        else { setSelectedOfficialPointId(null); setSelectedCustomPinId(null); _setEditingCustomPinIdRef(null); setDraftCustomPin(null) }
      }
    }
  })
  ...
  const saveCurrentRoute = useCallback(async () => { 
    ...
    if (isPub && isPub.owner !== userId) { showToast('Erro', 'Você não pode editar uma rota pública de outro usuário.', 'error'); return }
    try { 
  ...
  }, [isAuthenticated, user, currentRoute, customPins, showToast, selectedSavedRouteId, publicRoutes])

  const getPinTimerRemaining = useCallback((id: string, timer: number | null) => {
    const state = completedPins[id]; if (!state) return 0

    // Priorizar o timer do subtipo identificado, caso contrário usar o do ponto
    const actualTimer = (state.subType ? getResourceTimer(state.subType) : undefined) ?? timer

    if (actualTimer === null || actualTimer === undefined) return 0
    const elapsed = Math.floor((Date.now() - new Date(state.completedAt).getTime()) / 1000)
    return Math.max(0, actualTimer - elapsed)
  }, [completedPins])
  const selectCustomPin = useCallback((id: string | null) => {
    if (mode === 'route' && id) {
      const pin = customPins.find(p => p.id === id); if (pin) setCurrentRoute(prev => ({ ...prev, checkpoints: [...prev.checkpoints, { id: createId('checkpoint'), x: pin.x, y: pin.y, customPinId: pin.id, label: pin.name }] }));
      return
    }
    setSelectedCustomPinId(id); setSelectedOfficialPointId(null)
    if (id === null) { setDraftCustomPin(null); _setEditingCustomPinIdRef(null); _setMode('explore') }
  }, [mode, customPins, _setEditingCustomPinIdRef])

  const selectOfficialPoint = useCallback((id: string | null) => {
    if (!id) { setSelectedOfficialPointId(null); return }
    if (mode === 'route') {
      const p = officialPoints.find(i => i.id === id); if (p) setCurrentRoute(prev => ({ ...prev, checkpoints: [...prev.checkpoints, { id: createId('checkpoint'), x: p.x, y: p.y, pointId: p.id, label: p.name }] }));
      return
    }
    setSelectedOfficialPointId(id); setSelectedCustomPinId(null); _setEditingCustomPinIdRef(null); setDraftCustomPin(null)
  ...
  const confirmCustomPin = useCallback(() => {
    ...
    setCustomPins(prev => {
      const next = prev.find(p => p.id === draftCustomPin.id) ? prev.map(p => p.id === draftCustomPin.id ? draftCustomPin : p) : [...prev, draftCustomPin]
      localStorage.setItem('shinobi-map-custom-pins', JSON.stringify(next)); return next
    })
  ...
  const cancelCustomPin = useCallback(() => { 
    ...
    if (id && id.startsWith('pin-')) { setCustomPins(prev => prev.filter(p => p.id !== id)) }
    setDraftCustomPin(null); _setEditingCustomPinIdRef(null); _setMode('explore') 
  ...
  return {
    ...
    interactiveMap, isAuthenticated, 
    loadSavedRoute: useCallback(async (id: string) => { 
      try { 
        let r = savedRoutes.find(it => it.id === id);
        if (r) { setCurrentRoute(r.route); setSidebarSection('routes'); _setMode('route'); setSelectedSavedRouteId(id); if (!visibleRoutes.includes(id)) setVisibleRoutes(prev => [...prev, id]); return }
        r = publicRoutes.find(it => it.id === id);
        if (r) { setCurrentRoute(r.route); setSidebarSection('routes'); _setMode('explore'); setSelectedSavedRouteId(id); if (!visibleRoutes.includes(id)) setVisibleRoutes(prev => [...prev, id]); showToast('Info', 'Visualizando rota pública (Somente Leitura)', 'info') }
      } catch { showToast('Erro', 'Falha ao carregar.', 'error') } 
    }, [savedRoutes, publicRoutes, visibleRoutes, showToast]),
    unloadCurrentRoute: useCallback(() => { setSelectedSavedRouteId(null); setCurrentRoute({ checkpoints: [], color: '#00d6a3', createdAt: new Date().toISOString(), customPins: [], description: '', id: createId('route'), name: 'Nova rota', updatedAt: new Date().toISOString() }) }, []),
    mapLayers, mapRegions, markerTypes, mode, setMode: useCallback((m: 'explore' | 'pin' | 'route' | 'feedback') => _setMode(m), []),
    isFeedbackModalOpen, setIsFeedbackModalOpen, feedbackTarget, setFeedbackTarget,
    submitFeedback: useCallback(async (data: MapFeedback) => { 
      const userId = user?.id
      try { 
        await new SubmitMapFeedbackUseCase(mapDependencies.mapFeedbackRepository).execute({ ...data, user_id: userId }); 
        ...
        markerTypes.forEach(type => {
            ...
            if (isDyn) {
                ...
                pinsOfType.forEach(p => { 
                    const state = completedPins[p.id]
                    if (state?.subType) subTypeMap[state.subType] = (subTypeMap[state.subType] || 0) + 1 
                })
    ...
    removeCustomPin: useCallback((id: string) => { setCustomPins(prev => { const next = prev.filter(p => p.id !== id); localStorage.setItem('shinobi-map-custom-pins', JSON.stringify(next)); return next }) }, []),
    ...
    selectOfficialPoint,
    selectedCustomPin: draftCustomPin || customPins.find(p => p.id === selectedCustomPinId) || null,
    ...
    toggleCustomPinVisibility: useCallback((id: string) => { setCustomPins(prev => prev.map(p => p.id === id ? { ...p, isHidden: !p.isHidden } : p)) }, []),
    toggleRouteVisibility: useCallback((id: string) => { setVisibleRoutes(prev => prev.includes(id) ? prev.filter(i => i !== id) : [...prev, id]) }, []),
    visibleRoutes, updateRouteField: useCallback((f: keyof CustomRoute, v: string | string[] | number) => setCurrentRoute(prev => ({ ...prev, [f]: v })), []),
    updateSelectedPinField: useCallback((f: keyof SavedCustomPin, v: string | string[] | boolean | number) => { if (draftCustomPin) setDraftCustomPin({ ...draftCustomPin, [f]: v }) }, [draftCustomPin]),
    user, visibleCustomPins, visibleOfficialPoints, completedPins, togglePinCompleted, getPinTimerRemaining, globalTick, notificationSettings, updateNotificationSettings, isSettingsModalOpen, setIsSettingsModalOpen, requestPushPermission, mineRoutesPage, setMineRoutesPage,
    mapStats: currentStatsView,
    worldStats: useMemo(() => {
        const stats = { total_ores: 0, marked_ores: 0, total_mushrooms: 0, marked_mushrooms: 0, total_plants: 0, marked_plants: 0, total_sticks: 0, marked_sticks: 0 }
        officialPoints.forEach(p => {
            const isM = !!completedPins[p.id] && completedPins[p.id].status !== 'ready'
            if (p.type === 'ore') { stats.total_ores++; if (isM) stats.marked_ores++ }
            else if (p.type === 'mushroom') { stats.total_mushrooms++; if (isM) stats.marked_mushrooms++ }
            else if (['perpetual', 'hibiscus', 'cotton', 'borago'].includes(p.type)) { stats.total_plants++; if (isM) stats.marked_plants++ }
            else if (p.type === 'stick') { stats.total_sticks++; if (isM) stats.marked_sticks++ }
        })
        return stats
